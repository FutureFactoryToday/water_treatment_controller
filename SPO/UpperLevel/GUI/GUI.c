...
void translateMessage(void){
	...
	if (touchDelay == 0 && tsState.TouchDetected == 1)
    {
        ...
        switch(frame)
        {
            case 0:
            {
                if (isInRectangle(tsState.X,tsState.Y,DROP_BUT_X,DROP_BUT_Y,DROP_BUT_X + DROP_BUT_X_SIZE,DROP_BUT_Y + DROP_BUT_Y_SIZE) )
                {
                    if (dropBut == 0)
                    {
            ...
            case 1:
            {
                if (isInRectangle(tsState.X,tsState.Y,FIRST_CURSOR_POS_X,FIRST_CURSOR_POS_Y,FIRST_CURSOR_POS_X + FIRST_CURSOR_SIZE_X,FIRST_CURSOR_POS_Y + FIRST_CURSOR_SIZE_Y))
                {
                    frame = 11;
                }   
                if (isInRectangle(tsState.X,tsState.Y,SECOND_CURSOR_POS_X,SECOND_CURSOR_POS_Y,SECOND_CURSOR_POS_X + SECOND_CURSOR_SIZE_X,SECOND_CURSOR_POS_Y + SECOND_CURSOR_SIZE_Y))
                {
                    frame = 12;
                }   
                if (isInRectangle(tsState.X,tsState.Y,RETURN_BUT_POS_X,RETURN_BUT_POS_Y,RETURN_BUT_POS_X + RETURN_BUT_SIZE_X,RETURN_BUT_POS_Y + RETURN_BUT_SIZE_Y))
                {
                    frame = 0;
                }
                break;
...
//              }   
            ...
            case 12:
            {
                ...
                if (isInRectangle(tsState.X,tsState.Y,UP_ARROW_POS_X,UP_ARROW_POS_Y,UP_ARROW_POS_X + UP_ARROW_SIZE_X,UP_ARROW_POS_Y + UP_ARROW_SIZE_Y))
                {
                    if(wasScroll > 0) wasScroll--;
                }
                if (isInRectangle(tsState.X,tsState.Y,DOWN_ARROW_POS_X,DOWN_ARROW_POS_Y,DOWN_ARROW_POS_X + DOWN_ARROW_SIZE_X,DOWN_ARROW_POS_Y + DOWN_ARROW_SIZE_Y))
                {
