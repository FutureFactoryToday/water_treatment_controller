...
bool* FP_Manual_FRAM_Write(uint8_t* data, uint32_t adr){
	if (queue[0].set == 1)
		return HAL_BUSY;
	queue[0].set = 1;
	queueHandler();
}
bool* FP_Manual_FRAM_Read(uint8_t* data, uint32_t adr){
	
}
bool* FP_Manual_RAM_Write(uint8_t* data, uint32_t adr){
	
}
bool* FP_Manual_RAM_Read(uint8_t* data, uint32_t adr){
