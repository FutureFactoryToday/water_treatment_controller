#include "historyFilterFrame.h"

uint8_t history_filter_frame_Scroll_cnt = 0;
uint8_t history_filter_frame_was_Scroll = 0;
int8_t hwndHistoryFilterFrameControl = 0;
int8_t startHistoryFilterFrame = 0;
...
int ShowHistoryFilterFrame(void)
{
    history_filter_frame_Scroll_cnt = 0;
		/*Static create*/
		createFrame();
    while(1)
    {
...
void createFrame(void){
    ...
    drawMainBar(true, true, SMALL_LOGO_X, SMALL_LOGO_Y, MODE_HISTORY_FILTER);
...
//	BSP_LCD_DisplayStringAt(FIRST_CURSOR_POS_X + 9,FIRST_CURSOR_POS_Y + 9,ITEM_HISTORY_MENU[history_filter_frame_Scroll_cnt],LEFT_MODE);
//	BSP_LCD_DisplayStringAt(FIRST_CURSOR_POS_X + 9,SECOND_CURSOR_POS_Y + 9,ITEM_HISTORY_MENU[history_filter_frame_Scroll_cnt + 1],LEFT_MODE);
//	BSP_LCD_DisplayStringAt(FIRST_CURSOR_POS_X + 9,THRID_CURSOR_POS_Y + 9,ITEM_HISTORY_MENU[history_filter_frame_Scroll_cnt + 2],LEFT_MODE);
//	BSP_LCD_DisplayStringAt(FIRST_CURSOR_POS_X + 9,FOURTH_CURSOR_POS_Y + 9,ITEM_HISTORY_MENU[history_filter_frame_Scroll_cnt + 3],LEFT_MODE);
...
void calcButParam()
{
    TC_clearButtons();
        ...
        //Setting for key "0"
    menuLines[history_filter_frame_Scroll_cnt].x = FIRST_CURSOR_POS_X;
    menuLines[history_filter_frame_Scroll_cnt].y = FIRST_CURSOR_POS_Y;
    menuLines[history_filter_frame_Scroll_cnt].xSize = FIRST_CURSOR_SIZE_X;
    menuLines[history_filter_frame_Scroll_cnt].ySize = FIRST_CURSOR_SIZE_Y;
		
		//Setting for key "1"
    menuLines[history_filter_frame_Scroll_cnt + 1].x = SECOND_CURSOR_POS_X;
    menuLines[history_filter_frame_Scroll_cnt + 1].y = SECOND_CURSOR_POS_Y;
    menuLines[history_filter_frame_Scroll_cnt + 1].xSize = SECOND_CURSOR_SIZE_X;
    menuLines[history_filter_frame_Scroll_cnt + 1].ySize = SECOND_CURSOR_SIZE_Y;
    
		//Setting for key "2"
    menuLines[history_filter_frame_Scroll_cnt + 2].x = THRID_CURSOR_POS_X;
    menuLines[history_filter_frame_Scroll_cnt + 2].y = THRID_CURSOR_POS_Y;
    menuLines[history_filter_frame_Scroll_cnt + 2].xSize = THRID_CURSOR_SIZE_X;
    menuLines[history_filter_frame_Scroll_cnt + 2].ySize = THRID_CURSOR_SIZE_Y;
    
		//Setting for key "3"
    menuLines[history_filter_frame_Scroll_cnt + 3].x = FOURTH_CURSOR_POS_X;
    menuLines[history_filter_frame_Scroll_cnt + 3].y = FOURTH_CURSOR_POS_Y;
    menuLines[history_filter_frame_Scroll_cnt + 3].xSize = FOURTH_CURSOR_SIZE_X;
    menuLines[history_filter_frame_Scroll_cnt + 3].ySize = FOURTH_CURSOR_SIZE_Y;
    
    for (uint8_t i = history_filter_frame_Scroll_cnt; i < history_filter_frame_Scroll_cnt + 4; i++){
